...
var hasTouch = H.hasTouch, merge = H.merge, wrap = H.wrap, addEvent = H.addEvent, relativeLength = H.relativeLength,
    Axis = H.Axis, Pointer = H.Pointer,
...
H.AxisResizer.prototype = {
    ...
    render: function () {
        var resizer = this, axis = resizer.axis, chart = axis.chart, options = resizer.options, x = options.x,
            ...
            pos = Math.min(Math.max(axis.top + axis.height + y, chart.plotTop), chart.plotTop + chart.plotHeight),
            attr = {}, lineWidth;
        if (!chart.styledMode) {
            attr = {
